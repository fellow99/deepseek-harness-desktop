...
> BrowserWindow 创建、loadURL(localhost)/兜底页、无边框 + titleBarOverlay、安全加固、后台驻留的关窗处理。
...
> 进程级入口编排器：单实例锁、安装器短生命周期处理、启动编排（安全准备 → 宿主 → 窗口 → 托盘/通知）、生命周期（后台驻留 + 优雅退出）。
